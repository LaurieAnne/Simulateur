...
namespace Simulateur
{
    public class AvionCiterne : Vehicule
    {
        ...
        public AvionCiterne(string p_nom, int p_KMH, int p_tempsMain, int p_tempsCharg, int p_tempsLarg, Aeroport p_aeroport, Feu p_client) //Constructeur
        ...
        public override void ChangerEtat(object source)
        {
            if (m_client != null)
            {
                ...
                if (m_etat.ToString() == "Hangar")
                {
                    ...
                    m_etat = new AllerRetour(aeDepart, m_posActuelle, tempsVol, posDestination);
